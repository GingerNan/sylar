...
sylar::ConfigVar<std::unordered_set<int>>::ptr g_int_uset_value_config = 
    sylar::Config::Lookup("system.int_uset", std::unordered_set<int>{1, 2}, "system int unordered set");
...
#define XX_M(g_var, name, prefix) \
    ...
    XX_M(g_int_map_value_config, str_int_umap, after);
...
sylar::ConfigVar<std::map<std::string, std::vector<Person>>>::ptr g_person_vec_map =
    sylar::Config::Lookup("class.person_vec_map", std::map<std::string, std::vector<Person>>(), "system person map");
...
#define XX_MP(g_var, prefix) \
    ...
    XX_MP(g_person_map, "class.person_map after");
    SYLAR_LOG_INFO(SYLAR_LOG_ROOT()) << "g_person_vec_map before: " << g_person_vec_map->toString();
}
